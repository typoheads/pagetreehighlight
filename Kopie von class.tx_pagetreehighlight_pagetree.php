		...
		if (is_array($this->data)) {
			...
			if (is_array($row) && ($this->table=='pages') )	{
				if ($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['pagetreehighlight']['advancedHighlight'])	{
					$GLOBALS['SOBE']->doc->inDocStylesArray['tx-kbpthl-hl'] = '
	...
	background-color: #ebebeb;
				...
				if ( $GLOBALS['BE_USER']->uc['tx_pagetreehighlight']['page'][$row['uid']] || $GLOBALS['BE_USER']->uc['tx_pagetreehighlight']['branch'][$row['uid']] || $pagepreset || $branchpreset )	{
					...
					if (($preset = $GLOBALS['BE_USER']->uc['tx_pagetreehighlight']['page'][$row['uid']]) && is_array($colors[$preset]))	{
						$row['_CSSCLASS'] = 'tx-kbpthl-page-'.$preset;
						$GLOBALS['SOBE']->doc->inDocStylesArray['tx-kbpthl-page-'.$preset] = $this->getRowStyle($colors[$preset], $preset, 'page');
					} elseif (is_array($pagepreset)&&count($pagepreset))	{
						$row['_CSSCLASS'] = 'tx-kbpthl-page-preset-'.$row['uid'];
						$GLOBALS['SOBE']->doc->inDocStylesArray['tx-kbpthl-page-preset-'.$row['uid']] = $this->getRowStyle($pagepreset, 'preset-'.$row['uid'], 'page');
					}
					...
					if (($preset = $GLOBALS['BE_USER']->uc['tx_pagetreehighlight']['branch'][$row['uid']]) && is_array($colors[$preset]))	{
						if (!$row['_CSSCLASS'])	{
							$row['_CSSCLASS'] = 'tx-kbpthl-branch-'.$preset;
						}
						$row['_SUBCSSCLASS'] = 'tx-kbpthl-branch-'.$preset;
						$GLOBALS['SOBE']->doc->inDocStylesArray['tx-kbpthl-branch-'.$preset] = $this->getRowStyle($colors[$preset], $preset, 'branch');
					} elseif (is_array($branchpreset)&&count($branchpreset))	{
						if (!$row['_CSSCLASS'])	{
							$row['_CSSCLASS'] = 'tx-kbpthl-branch-preset-'.$row['uid'];
						}
						$row['_SUBCSSCLASS'] = 'tx-kbpthl-branch-preset-'.$row['uid'];
						$GLOBALS['SOBE']->doc->inDocStylesArray['tx-kbpthl-branch-preset-'.$row['uid']] = $this->getRowStyle($branchpreset, 'preset-'.$row['uid'], 'branch');
					}
				}
			}
			return $row;
...
.tx-kbpthl-'.$type.'-'.$key.' ul {
						'.$def['allstyles'].'
					}
';
			if ($type == 'page') {
				$styles .= '
ul.tree ul li.tx-kbpthl-'.$type.'-'.$key.' ul {
...
.tx-kbpthl-'.$type.'-'.$key.' ul, .expanded ul{
